...
export function accumulatedCommission(sales: readonly Sale[], commissionRate: number): number {
...
export function remainingCommission(
  ...
  return Math.max(0, monthlyCommissionGoal - accumulatedCommission(sales, commissionRate));
}
...
export function grossSalesNeededForCommissionGoal(
  ...
  return Math.ceil(remainingCommission(sales, commissionRate, monthlyCommissionGoal) / commissionRate);
}
...
export function estimatedSalesNeededForCommissionGoal(
  ...
  return Math.ceil(grossSalesNeededForCommissionGoal(sales, commissionRate, monthlyCommissionGoal) / average);
}
...
export function goalProgress(
  ...
  return Math.min(
    100,
    Math.round((accumulatedCommission(sales, commissionRate) / monthlyCommissionGoal) * 100)
  );
}
